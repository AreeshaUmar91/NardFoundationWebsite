        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-[760px]:grid-cols-1 max-[760px]:gap-6">
          {projects.map((project, index) => {
            ...
            return (
              <div
                key={project.id}
                className="flex flex-col justify-between bg-transparent p-0 max-[760px]:w-full max-[760px]:mx-auto"
              >
                {/* Project Image */}
                <div className="w-full relative">
                  ...
                  {isComingSoon && (
                    <div className="absolute inset-0 bg-[#D9D9D9B2] flex items-center justify-center">
                      <span className="text-black text-xl font-semibold max-[760px]:text-lg">
                        Coming Soon
                      </span>
                    </div>
                  )}
                ...
                {/* Project Title */}
                <p
                  className="font-semibold text-gray-900 mt-4 mb-4 text-base sm:text-lg max-[1100px]:text-base max-[760px]:text-sm"
                  style={{ lineHeight: "1.4" }}
                  dangerouslySetInnerHTML={{ __html: project.title }}
                ></p>

                {/* Buttons */}
                <div className="flex justify-between gap-3 max-[760px]:flex-col max-[760px]:gap-2 max-[760px]:w-full">
                  <button className="h-12 flex-[2] px-[50px] rounded-3xl bg-[#E8E8E8] flex items-center justify-between font-bold text-sm sm:text-base max-[760px]:flex-[1] max-[760px]:px-6 max-[760px]:w-full">
                    <span>Donation:</span>
                    <span>{project.donation}</span>
                  </button>

                  <button className="h-12 flex-[1] px-4 rounded-3xl text-white bg-[#263750] font-normal text-sm sm:text-base max-[760px]:flex-[1] max-[760px]:px-6 max-[760px]:w-full">
                    CONTRIBUTION
                  </button>
                </div>
              </div>
